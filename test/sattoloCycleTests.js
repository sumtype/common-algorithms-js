...
// Algorithm
const sattoloCycle = require(path.join(__dirname, '..', 'algorithms', 'sattoloCycle.js'))
...
describe('Sattolo Cycle Algorithm: sattoloCycle(array)', () => {
  it('Should return the input array with its indices\' values shuffled such that none are in the same position.', () => {
    let array = [10, 9, 8, 6, 3, 15, 168, 186, 298, 2, 24, 4, 26], shuffledArray = sattoloCycle(array)
    for (let i = 0; i < array.length; i++) expect(shuffledArray[i].toString()).to.not.eql(array[i].toString())
    array = [10, 9, 8], shuffledArray = sattoloCycle(array)
    for (let i = 0; i < array.length; i++) expect(shuffledArray[i].toString()).to.not.eql(array[i].toString())
    array = [10, 9], shuffledArray = sattoloCycle(array)
    for (let i = 0; i < array.length; i++) expect(shuffledArray[i].toString()).to.not.eql(array[i].toString())
  })
